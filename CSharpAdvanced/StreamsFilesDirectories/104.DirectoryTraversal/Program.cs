...
namespace _104.DirectoryTraversal
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Dictionary<string, Dictionary<string, double>> sortedDict =
                filesData.OrderByDescending(kvp => kvp.Value.Count).ThenBy(kvp => kvp.Key).ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
            ...
            string finalPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "output.txt");
